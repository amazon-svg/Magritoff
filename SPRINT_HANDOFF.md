...
**Reste ouvert sur E10.18** : **E10.18e** (l'écran, et la reprise avec Arnaud du jeu de colonnes/taille de page de la grille de (a) ; y montrer un vrai fichier — les taux s'y lisent `0,2000` et non `20 %`, choix de l'architecte réversible par le seul style de cellule) ; (a2) comportement de `pg_net` depuis une fonction `security definer` — bloque le lot (f) seul, optionnel, **a mesurer plutot qu a lire** ; (j) dette `shopExport.ts` (export catalogue cote navigateur, agregation et TVA calculees dans le navigateur — **precedent a ne pas suivre**, sans rapport avec E10.18).
...
- **E10.18** — Export XLSX et CSV des commandes pour la comptabilite — **lots a/b/c/d LIVRES ET DEPLOYES au 2026-09-13** ; reste **(e) l'écran** (prochain lot naturel) et (f) optionnel. Le runner reste inerte jusqu'à la porte d'activation (export XLSX de 5 000 lignes sur le projet hébergé)
...
**CABLAGE NON ENCORE VERIFIE** : le declenchement manuel de verification a ete bloque par le classificateur de permissions. **A controler apres le premier tour (2026-09-14 05:00 UTC)** : `cron.job_run_details` du job, et la reponse HTTP dans `net._http_response` (200 attendu ; 401 = secret fonction/Vault desaligne). Reserve encore ouverte : domaine d expedition Resend verifie (sans lui, les rappels J+10/J+15 ne partent pas et la purge J+30 reste bloquee — sans effet aujourd hui, aucun espace n a la purge armee).
