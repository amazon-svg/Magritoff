...
- `main` est tenu **fast-forward synchronise** avec cette branche apres CHAQUE story approuvee et deployee (jamais de merge --no-ff, jamais de commit sur `main` directement). Dernier commit commun aux deux : `fbab5c77 feat(v5): E10.15d-1 - branchement quote.sent/quote.converted/customer.created + ecran du journal`.
...
Decoupage arrete par l architecte (docs/api/CONVENTIONS.md §8.23) : (a) socle — FAIT, (b) ecran — FAIT, (c) chaine d envoi complete sur `order.step_changed` + journal + retention — FAIT ET DEPLOYE, (d) reste du catalogue d evenements — **d-1 FAIT ET DEPLOYE** (quote.sent/quote.converted/customer.created + ecran journal, voir ci-dessus), **d-2 A FAIRE** (order.files_submitted + mecanisme de rendu differe), (e) canal SMS.

**Reserve (i) LEVEE par l architecte le 2026-09-12** (§8.23 point 11, nouveau) : `{{files.count}}` sera rendu **differe**, au moment de la remise reelle (pas a la mise en file), et **scelle** dans le journal par la meme transition d etat qui fait sortir le message de `pending` — la valeur de preuve du journal reste donc reelle sur tout statut terminal. Nouveau champ contrat `NotificationTag.render_stage` (`enqueue`/`delivery`, `files.count` seule `delivery`) deja complete cote catalogue en d-1 (metadonnee uniquement). **Reste a faire en E10.15d-2** : migration SQL additive (colonne `deferred_render`, reecriture du trigger d immuabilite pour autoriser `subject`/`body` a changer UNIQUEMENT `pending → statut terminal`, refus de tout retour a `pending`, `api_enqueue_notification_message` a recreer avec un parametre de plus — `drop`+`create`, pas `create or replace`, car la signature change), branchement d `order.files_submitted` sur `NotificationDispatchConsumer`, logique de scellage dans `NotificationSender`. Detail complet au §8.23 point 11 (11.1 a 11.5).
