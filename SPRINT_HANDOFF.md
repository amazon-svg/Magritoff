...
**Reste ouvert sur E10.18** : **E10.18e-2** (l'export depuis la grille : modale, reprise des filtres actifs, panneau des exports ; cadré au §8.24, **déploiement conditionné à l'activation du générateur**, décision Arnaud (k) du 2026-09-14 ; y montrer un vrai fichier — les taux s'y lisent `0,2000` et non `20 %`, réversible par le seul style de cellule) ; (a2) comportement de `pg_net` depuis une fonction `security definer` — bloque le lot (f) seul, optionnel, **a mesurer plutot qu a lire** ; (j) dette `shopExport.ts` (export catalogue cote navigateur, agregation et TVA calculees dans le navigateur — **precedent a ne pas suivre**, sans rapport avec E10.18).
...
- **E10.18** — Export XLSX et CSV des commandes pour la comptabilite — **lots a/b/c/d LIVRES ET DEPLOYES au 2026-09-13, e-1 (grille + menu) LIVRE le 2026-09-14** ; reste **e-2 (l'export depuis la grille)**, dont le déploiement attend l'activation du générateur, et (f) optionnel. Le runner reste inerte jusqu'à la porte d'activation (export XLSX de 5 000 lignes sur le projet hébergé)
