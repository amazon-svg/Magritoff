...
**Constat trouve en corrigeant ce point, hors perimetre de Q17-c, a traiter separement** : `src/types/database.types.ts` (committe) est perime d'environ 95 tables entieres par rapport a la pile locale — tout le module E10 gestion commerciale (`commercial_orders`, `commercial_quotes`, `price_rules`, `client_price_rules`, `production_steps`, `notification_logs`, `outbox_events`, la plupart des fonctions `api_*`, etc.) est absent du fichier genere alors qu'il existe et fonctionne en base (le code applicatif contourne deja ce manque par des elargissements de type manuels au cas par cas, comme celui deja present dans `orders-repository.ts` avant ce lot). Une regeneration complete (`pnpm db:types`, PAT requis, ou `supabase gen types typescript --local`) apporterait un diff enorme et sans rapport avec ce lot : **remonte ici comme dette a traiter par un lot dedie**, pas resolue en silence par Q17-c.
