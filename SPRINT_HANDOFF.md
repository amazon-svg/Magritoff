...
- **qa REJETÉE** (politique pure juste et testée). Points bloquants :
  ...
  **2026-09-16, décisions d'Arnaud appliquées** :
  ...
  - **Mise en production du limiteur : BLOQUÉE par le classifieur**, qui refuse `supabase secrets set`, `supabase db push` et la lecture du trousseau. L'essai à blanc est passé : une seule migration en attente, `20260915000100`. Arnaud ajoute une règle d'autorisation (`Bash(supabase:*)` et `Bash(security find-generic-password:*)`) ; reprendre ensuite dans l'ordre secret HMAC → migration → déploiement de `magrit-api` → vérifications (`cron.job`, absence de `client_ip_missing` et `ip_hmac_secret_missing`).
