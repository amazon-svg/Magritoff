...
- `main` est tenu **fast-forward synchronise** avec cette branche apres CHAQUE story approuvee et deployee (jamais de merge --no-ff, jamais de commit sur `main` directement). Dernier commit commun aux deux : `56642608 feat(v5): E10.18c - ressource, file d attente, runner et renderer CSV de l export`.
...
**Reste ouvert sur E10.18** : (a2) comportement de `pg_net` depuis une fonction `security definer` — bloque le lot (f) seul, optionnel, **a mesurer plutot qu a lire** ; (j) dette `shopExport.ts` (export catalogue cote navigateur, agregation et TVA calculees dans le navigateur — **precedent a ne pas suivre**, sans rapport avec E10.18).
...
- **E10.18** — Export XLSX et CSV des commandes pour la comptabilite (P1, effort M, jamais cadree) — **la seule story du sprint encore entierement ouverte**, prochaine candidate naturelle
...
### Activation du mecanisme de purge (E10.22a-d **ET E10.18c**) — geste separe, jamais fait
...
⚠️ **PORTEE ELARGIE PAR E10.18c, a savoir avant d activer ou de ne pas activer** : `magrit-order-file-purge` ne fait plus seulement le menage des fichiers de commande — elle porte desormais AUSSI la **retention 7 jours des exports comptables** (arbitrage Arnaud), avec son balayage d objets orphelins dedie. **Consequence directe : tant que ce `pg_cron` n est pas pose, AUCUN export comptable n est jamais detruit** — et ces fichiers portent, en clair et hors RLS, le chiffre d affaires complet, les SIRET et les numeros de TVA de tous les clients d un espace. La retention de 7 jours est **codee, testee et prouvee par mutation sur quatre rounds de revue** ; elle n est simplement **pas en vigueur**. C est le point d activation le plus consequent des trois mecanismes inertes du sprint (notifications, purge de fichiers, exports), parce que c est le seul dont l inaction **accumule** de la donnee sensible au lieu de simplement ne rien faire.
