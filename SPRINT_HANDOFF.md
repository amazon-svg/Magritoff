...
- `main` est tenu **fast-forward synchronise** avec cette branche apres CHAQUE story approuvee et deployee (jamais de merge --no-ff, jamais de commit sur `main` directement). Dernier commit commun aux deux : `560621fb feat(v5): E10.15c - chaine d envoi des notifications sur order.step_changed`.
...
Decoupage arrete par l architecte (docs/api/CONVENTIONS.md §8.23) : (a) socle — FAIT, (b) ecran — FAIT, (c) chaine d envoi complete sur `order.step_changed` + journal + retention — **FAIT ET DEPLOYE** (voir ci-dessus), (d) reste du catalogue d evenements — **PAS COMMENCE**, (e) canal SMS.

**Reserve (i) ouverte par l architecte, bloquante pour (d), sans effet sur (c)** : `{{files.count}}` (regroupement de `order.files_submitted`) contredit l immuabilite du corps de message — a l insertion `occurrence_count` vaut 1, donc un corps fige annoncerait « 1 fichier » pour un lot de cinq receptionnes ensuite. Trois issues ecrites au §8.23 point 9, preference de l architecte pour assouplir l immuabilite de `subject`/`body` tant que le message est `pending`. **A trancher par Arnaud avant qu E10.15d demarre.**
