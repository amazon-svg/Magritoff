...
| **S4.1b** Pipeline rendu SVG → PNG + 1er template flyer | (à committer) | Module `supabase/functions/_shared/mockup/` (types + renderer + 1 template + 5 tests Deno). **Pivot technique majeur vs Architecture §4.3** : `sharp + svgdom` (Node native) abandonnés (incompat Deno Deploy). Pivot `npm:@resvg/resvg-wasm@2.6.2` (pure WASM, fonts Inter/Bitter/JetBrains incluses). Init WASM lazy via fetch unpkg. Snapshot SVG versionné dans `templates/flyer.snapshot.svg`. Tests Deno 5/5 passants (975ms total). Perf : 1er render warm-cache 784ms (init WASM), suivants 183ms (cible 500ms p50 OK). Vitest 37/37 (0 régression). |
