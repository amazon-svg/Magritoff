...
**Round suivant (`0cc54e27`) : le nettoyeur passe au lexer TypeScript, MAIS une desynchronisation reelle a ete mesuree sur un fichier reel (Q17-c, `OrderHistoryTable.tsx`)**, pas seulement sur les fragments courts du fichier de test du helper. Comparaison caractere par caractere entre le source brut et la sortie de `stripComments()` sur ce fichier NON modifie : le dernier ecart tombe a l'index 24362, ligne 647 (un commentaire JSX `{/* ─── ... */}` avec des caracteres Unicode de dessin de boite) — **au-dela, `stripComments` ne modifie plus rien sur ce fichier**, y compris un commentaire reel preexistant (lignes 1066-1068) qui reste intact dans la sortie nettoyee. Consequence mesuree sur `OrderHistoryTable.wiring.test.ts` : deux mutations sur trois, sous forme de commentaire de bloc COLLE a une parenthese, survivent (le test reste vert) ; la troisieme, plus loin dans le fichier, est capturee. Cause precise non isolee (probablement le suivi de contexte JSX qu'un simple `scan()` en boucle ne gere pas comme le ferait le parseur complet, via `reScanJsxToken`/`scanJsxToken`). **Signale, pas contourne, pas corrige** : voir `_bmad-output/implementation-artifacts/story-Q17c.md`, section ROUND 7, pour la reproduction complete. Ce nettoyeur est desormais un point de defaillance unique pour plusieurs gardes du chantier E10 (Q14-a, Q17-c, Q20) : cette desynchronisation sur fichier reel merite d'etre rejouee sur les autres fichiers porteurs avant de considerer le nettoyeur fiable a l'echelle du chantier.
