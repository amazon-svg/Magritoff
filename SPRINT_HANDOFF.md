...
- `main` est tenu **fast-forward synchronise** avec cette branche apres CHAQUE story approuvee et deployee (jamais de merge --no-ff, jamais de commit sur `main` directement). Dernier commit commun aux deux : `53ad7bb2 feat(v5): E10.18b - les deux vues privees du jeu de donnees de l export`.
...
**Prochain lot : E10.18c** — la ressource, la file et le renderer CSV : table `commercial_order_exports` + trigger + RLS testee, `api_request_order_export`, `api_claim_order_exports`, `api_read_order_export_rows` (descendue du lot (b)), bucket, purge `pg_cron` 7 jours, les trois endpoints, le module, le runner. **C est le lot ou un service comptable recoit pour la premiere fois un vrai fichier**, et celui ou l isolation de tenant se prouve enfin.
