...
| IA API-first | `MAGRIT_AI_PROVIDER=anthropic|openai|mistral`, clé dédiée par fournisseur et modèle optionnel `MAGRIT_AI_MODEL`. Diagnostic et éditorial de catégorie utilisent cette sélection. Seul le chat SSE historique reste isolé derrière `AssistantGateway`. |
