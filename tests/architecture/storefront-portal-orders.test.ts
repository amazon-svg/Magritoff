...
const portal = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/storefront/PortalOrders.tsx'), 'utf8');
const storefront = readFileSync(resolve(process.cwd(), 'src/modules/shops/ui/storefront/PublicShop.tsx'), 'utf8');
const storefrontCatalog = readFileSync(resolve(process.cwd(), 'src/modules/shops/ui/hooks/usePublicShopCatalog.ts'), 'utf8');
const storefrontOrders = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/hooks/useStorefrontOrderLifecycle.ts'), 'utf8');
const storefrontOrderList = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/hooks/useStorefrontOrderList.ts'), 'utf8');
const storefrontOrderReceipt = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/hooks/useStorefrontOrderReceipt.ts'), 'utf8');
const storefrontOrderEditor = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/hooks/useStorefrontOrderEditor.ts'), 'utf8');
const editor = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/storefront/PortalOrderEditor.tsx'), 'utf8');
const thankYou = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/storefront/PortalThankYou.tsx'), 'utf8');
const auditModal = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/storefront/OrderAuditTrailModal.tsx'), 'utf8');
const dashboard = readFileSync(resolve(process.cwd(), 'src/modules/orders/ui/workspace/OrdersPage.tsx'), 'utf8');
const storefrontRuntime = readFileSync(resolve(process.cwd(), 'src/platform/runtime/storefront-ui-runtime.tsx'), 'utf8');
...
const storefrontEditorial = readFileSync(resolve(process.cwd(), 'src/modules/catalog/ui/hooks/useStorefrontCategoryEditorial.ts'), 'utf8');
...
describe('portail commandes par compte boutique', () => {
  ...
  it('utilise un transport sans bearer Magrit pour toutes les commandes storefront', () => {
    ...
    expect(storefrontOrderReceipt).toContain('useStorefrontApi(OrdersApiClient)');
  ...
  it('charge aussi le catalogue boutique sans bearer Magrit', () => {
    ...
    expect(storefrontCatalog).toContain('useStorefrontApi(ShopsApiClient)');
  ...
  it('ne réutilise pas le bearer Magrit pour l éditorial facultatif', () => {
    ...
    expect(storefrontEditorial).toContain('useStorefrontApi(DiagnosticsApiClient)');
  ...
  it('sépare le port client storefront du runtime workspace', () => {
    expect(storefrontRuntime).toContain('StorefrontUiRuntimeProvider');
    expect(storefrontRuntime).toContain('useStorefrontApi');
  ...
  it('monte les identités uniquement sur leurs routes respectives', () => {
    expect(appRoutes).toContain('import("@/app/surfaces/StorefrontRuntimeBoundary")');
    expect(appRoutes).toContain('import("@/app/surfaces/WorkspaceRuntimeBoundary")');
    ...
    expect(appRoutes).toContain('import("@/app/AppShell")');
    expect(appRoutes).toContain('lazyRoute(<AppShell />)');
    ...
    expect(storefrontBoundary).toContain('<StorefrontUiRuntimeProvider value={value}>');
    ...
    expect(workspaceBoundary).toContain('<ApiRuntimeProvider>');
  });
});
