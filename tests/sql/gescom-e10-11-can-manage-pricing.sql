...
-- que sur `tm.role in ('admin', 'member')`.
-- ----------------------------------------------------------------------------
...
-- du simple statut de membre. La retention d acces des `admin` (derivation,
-- inchangee par cette migration) reste couverte par les controles positifs
-- deja existants de gescom-e10-6-price-rules.sql (acteur `role = 'admin'`) :
-- si la derivation etait cassee, ce fichier-la echouerait deja.
--
...
--      ecritures redeviennent refusees — la policy lit `user_has_capability`
--      A CHAQUE APPEL, pas un instantane fige a l affectation.
--
...
begin
  ...
  -- toute reussite d ecriture ci-dessous ne puisse venir QUE du droit dedie,
  -- jamais de la derivation d appartenance.
  ...
  insert into e10_11_pricing_context (actor_id, tenant_a, gamme_a)
  values (v_actor, v_tenant_a, v_gamme_a);
end;
$$;
...
begin
  ...
  exception
    when insufficient_privilege or others then v_margin_rejected := true;
  end;
...
--      pas fige a l affectation — l ecriture redevient refusee. ───────────
