...
begin
  ...
  insert into auth.users (id, email, encrypted_password, email_confirmed_at, created_at, updated_at, aud, role)
    values (v_actor, 'e10-3-commercial-quotes-owner@example.test', 'x', now(), now(), now(), 'authenticated', 'authenticated');
  ...
  -- protect_last_tenant_admin (migration 20260824000200, UM1 — "le dernier
  -- moyen d administrer ne peut pas disparaitre") refuse desormais la
  -- suppression du DERNIER admin d un tenant. La Phase 2 plus bas retire
  -- v_actor du tenant A pour exercer la RLS d isolation (scenarios 4-6) ;
  -- sans un second admin, ce retrait serait lui-meme bloque par le
  -- trigger. Ce membre de reserve n intervient dans aucune assertion.
  v_tenant_a_spare_admin := gen_random_uuid();
  insert into auth.users (id, email, encrypted_password, email_confirmed_at, created_at, updated_at, aud, role)
    values (v_tenant_a_spare_admin, 'e10-3-commercial-quotes-tenant-a-spare-admin@example.test', 'x', now(), now(), now(), 'authenticated', 'authenticated');
  insert into public.tenant_members (tenant_id, user_id, role, access_scope, allowed_shop_ids)
