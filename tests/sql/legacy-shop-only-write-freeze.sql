...
begin
  ...
  select id into v_buyer_role
   ...
   where tenant_id = v_tenant and name = 'Acheteur';
  if v_buyer_role is null then
    raise exception 'Rôle Acheteur historique requis pour le scénario UM8.3';
  end if;
...
begin
  ...
  if not v_role_rejected then
    raise exception 'Une invitation Magrit a encore propagé le rôle Acheteur';
  end if;
