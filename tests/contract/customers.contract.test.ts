...
import type { CustomerContactDto, CustomerDto } from '@/modules/customers/api/contracts';
  ...
  it('CA5 — un interlocuteur cree ne porte aucune trace de compte utilisateur ou d invitation', async () => {
    ...
    expect(Object.keys(body.data).sort()).toEqual(
      [
        ...
        'phone',
        'role',
        // E10.5 — reflete les acces boutique REELLEMENT ouverts (CA3) ;
        // vide ici, car aucune action explicite d ouverture n a ete jouee.
        // N est PAS une trace de compte cree par defaut (CA5 reste vrai).
        'shop_accesses',
        'updated_at',
      ].sort(),
    );
    expect((body.data as { shop_accesses: unknown[] }).shop_accesses).toEqual([]);
  });
