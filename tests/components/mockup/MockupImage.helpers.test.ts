...
describe("buildPublicMockupUrl", () => {
  it("construit l'URL publique CDN avec le path correct (cache version _v2)", () => {
    ...
    expect(url).toBe(
      `https://${TEST_PROJECT_ID}.supabase.co/storage/v1/object/public/product_mockups/tenant-1/shop-1/product-1_v4.png`,
    );
  });
  ...
  it("ne fait pas d'encoding URL des params (ils sont attendus deja safes)", () => {
    ...
    expect(url).toContain("/abc-123/xyz-456/p-789_v4.png");
  });
});
