...
test('clic sur une ligne de devis ouvre bien l editeur du devis, pas un retour projets', async ({
  ...
  await quoteModal.getByRole('button', { name: 'Ajouter au projet', exact: true }).first().click();
  ...
  const projectId = await select.locator('option').nth(1).getAttribute('value');
  ...
  const itemRow = page.getByTestId('project-item-row').filter({ hasText: PRODUCT_LABEL }).first();
  ...
  const quoteId = page.url().split('/commercial-quotes/')[1];
  ...
  await page.goto(`/t/${TENANT_SLUG}/dashboard/quotes`);
  await page.waitForLoadState('networkidle', { timeout: 15_000 }).catch(() => {});
  const cleanupRow = page.locator(`[data-testid="quote-list-row"][data-quote-id="${quoteId}"]`);
  if (await cleanupRow.isVisible({ timeout: 5_000 }).catch(() => false)) {
  ...
  await page.goto(`/t/${TENANT_SLUG}/dashboard/projects/${projectId}`);
  await page.waitForLoadState('networkidle', { timeout: 15_000 }).catch(() => {});
  // Boucle (pas juste un essai) : des rejeux precedents de ce test, avortes
  // avant d atteindre ce nettoyage, ont pu laisser plusieurs items du meme
  // libelle sur ce projet reel partage.
  for (;;) {
    const candidate = page.getByTestId('project-item-row').filter({ hasText: PRODUCT_LABEL }).first();
    if (!(await candidate.isVisible({ timeout: 3_000 }).catch(() => false))) break;
    const itemId = await candidate.getAttribute('data-item-id');
    await candidate.getByTestId('project-item-remove-btn').click();
    await expect(page.locator(`[data-testid="project-item-row"][data-item-id="${itemId}"]`)).toBeHidden({
      timeout: 10_000,
    });
  }
});
