...
import { catalogRemovalResultSchema, gammeSubscriptionsSchema, pimCatalogSchema, pimDefinitionSchema, pimGammeSchema, setGammeSubscriptionsCommandSchema, upsertPimDefinitionCommandSchema, upsertPimGammeCommandSchema } from '../../modules/catalog/api/contracts.ts';
...
async function execute<T>(operation: () => Promise<T>): Promise<T> { try { return await operation(); } catch (error) { if (error instanceof CatalogRejectedError) { const status = error.code === 'invalid_request' ? 422 : error.code === 'not_found' ? 404 : error.code === 'conflict' ? 409 : 403; throw new ApiHttpError({ type: 'about:blank', title: status === 422 ? 'Catalogue invalide' : status === 404 ? 'Ressource catalogue introuvable' : status === 409 ? 'Conflit catalogue' : 'Gestion du catalogue interdite', status, code: `catalog.${error.code}`, detail: error.message }); } throw error; } }
