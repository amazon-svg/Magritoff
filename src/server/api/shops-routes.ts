...
import { createShopCommandSchema, createShopProductCommandSchema, mockupTemplateTypeSchema, mockupViewSchema, persistAiShopProductCommandSchema, persistAiShopProductResultSchema, publicShopCatalogSchema, publicShopProbeSchema, setShopPricingCommandSchema, shopBrandAssetKindSchema, shopBrandAssetResultSchema, shopBuyerRegistrationResultSchema, shopCustomMockupMutationResultSchema, shopCustomMockupsSchema, shopMutationResultSchema, shopPricingMutationResultSchema, shopPricingOverridesSchema, shopProductSchema, shopProductsSchema, shopRemovalResultSchema, shopSchema, tenantShopsSchema, updateShopCommandSchema, updateShopProductCommandSchema } from '../../modules/shops/api/contracts.ts';
...
export function createShopsRoutes(
  ...
  return [
    defineJsonRoute({ method: 'POST', path: `${API_V1_BASE_PATH}/shops/{shopId}/buyer-registration`, authentication: 'required', inputSchema: null, outputSchema: shopBuyerRegistrationResultSchema, async handle(context) { return execute(async () => { await service.registerBuyer(actor(context), param(context, 'shopId')); return { status: 200, body: { registered: true as const } }; }); } }),
