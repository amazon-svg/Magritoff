...
import { resolveProductGamme, resolveDefinition } from '@/modules/catalog/ui/helpers/productEnrichment';
import { resolveProductMockupAsset } from '@/modules/mockups/ui/helpers';
...
export function resolveProductImage(input: ResolveImageInput): string {
  ...
  if (input.gammes && input.definitions) {
    ...
    const gamme = resolveProductGamme(
      { config, name: input.name, ...(input.gamme_slug === undefined ? {} : { gamme_slug: input.gamme_slug }) },
      input.gammes,
    );
    if (gamme) {
