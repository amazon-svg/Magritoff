...
import type { Gamme } from '@/modules/catalog/ui/helpers';
import type { StorefrontSession } from "@/modules/shop-customers";
import type { PortalView, BudgetInfo } from '@/modules/orders/ui/storefront';
import { ShopMegaMenu } from '@/modules/catalog/ui/storefront';
import { ShopHeaderSearch } from '@/modules/catalog/ui/storefront';
...
interface Props {
  ...
  gammes?: GammePill[] | undefined;
  /** Set des slugs de gammes filtres actifs (multi). */
  activeGammeSlugs?: Set<string> | undefined;
  /** Toggle d'une gamme. */
  onToggleGamme?: ((slug: string) => void) | undefined;
  /** S2.18 — Taxonomie familles/sous-catégories pour le méga-menu. */
  taxonomy?: TaxonomyFamily[] | undefined;
  /** S2.18/S7.7 — Clic famille méga-menu (familyKey → page gamme). */
  onSelectFamily?: ((gammeSlugs: string[], familyKey?: string) => void) | undefined;
  /** S2.18 — Clic sous-catégorie dans le méga-menu → gammes à filtrer (+ format). */
  onSelectSubcategory?: ((gammeSlugs: string[], formatKey?: string) => void) | undefined;
  /** Contenu drawer panier (slide-right via Sheet). */
  cartDrawer?: ReactNode | undefined;
  /**
