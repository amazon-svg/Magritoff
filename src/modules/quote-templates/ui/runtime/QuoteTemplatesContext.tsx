...
export function QuoteTemplatesProvider({ children }: { children: ReactNode }) {
  const { user } = useAuth();
  const templatesApi = useWorkspaceApi(QuoteTemplatesApiClient);
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setCustomTemplates(overview.templates as QuoteTemplate[]);
  ...
  const cloneBuiltin = async (
    ...
    const { id: _id, builtin: _builtin, ...copy } = source;
