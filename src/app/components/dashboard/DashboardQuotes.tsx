...
type FilterKey = 'all' | 'draft' | 'sent' | 'won' | 'lost';

const STATUS_LABEL: Record<string, { label: string; cls: string }> = {
  draft:    { label: 'brouillon', cls: 'bg-[#F5F5F5] text-ink-muted' },
  sent:     { label: 'envoyé',    cls: 'bg-info-bg text-info-fg' },
  won:      { label: 'gagné',     cls: 'bg-ok-bg text-ok-fg' },
  lost:     { label: 'perdu',     cls: 'bg-err-bg text-err-fg' },
  pending:  { label: 'en cours',  cls: 'bg-warn-bg text-warn-fg' },
};
...
export function DashboardQuotes() {
  ...
  const kpis = useMemo(() => {
    ...
    const won30 = last30.filter((q) => q.status === 'won').length;
    const convRate = last30.length > 0 ? Math.round((won30 / last30.length) * 100) : 0;

    const avgBasket = quotes.length > 0
      ? quotes.reduce((s, q) => s + (q.total_ttc || 0), 0) / quotes.length
      : 0;

    const last7 = quotes.filter((q) => new Date(q.created_at).getTime() >= d7);
    const newLast7 = last7.length;

    // Sparklines sur 7 jours : nombre de devis par jour
  ...
  const visible = useMemo(() => {
    return quotes.filter((q) => {
      ...
      if (search.trim()) {
        ...
        const hay = `${q.reference} ${q.product_name}`.toLowerCase();
  ...
  const counts = useMemo(() => {
    const c: Record<FilterKey, number> = { all: quotes.length, draft: 0, sent: 0, won: 0, lost: 0 };
    for (const q of quotes) {
      if (q.status in c) c[q.status as FilterKey]++;
    }
    return c;
  }, [quotes]);
  ...
  return (
    <div className="max-w-[1400px]" style={{ fontFamily: 'var(--font-ui)' }}>
      {/* ── Titre + actions (topbar contextuelle) ─────────────────────── */}
      ...
      <div className="grid grid-cols-4 gap-px bg-line border border-line rounded-md overflow-hidden mb-5">
        <Kpi
          label="En cours"
          value={`${kpis.inProgress.toFixed(0)} €`}
          delta={`+${kpis.newLast7} · 7j`}
          deltaPositive
          spark={kpis.sparkDays}
          max={kpis.maxSpark}
        />
        <Kpi
          ...
          value={`${kpis.convRate}%`}
          delta={`30 j`}
          deltaPositive={kpis.convRate >= 50}
          spark={kpis.sparkDays}
          max={kpis.maxSpark}
        />
        <Kpi
          label="Panier moyen"
          value={`${kpis.avgBasket.toFixed(0)} €`}
          delta={`${quotes.length} devis`}
          deltaPositive
          spark={kpis.sparkDays}
          max={kpis.maxSpark}
        />
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border transition-colors ${
                active
                  ...
                  : 'bg-paper text-ink-muted border-line hover:text-ink hover:border-line-2'
              }`}
        ...
        <button
          className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md border border-dashed border-line-2 text-ink-muted hover:text-ink"
          ...
          <table className="w-full" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr className="border-b border-line bg-bg">
                {['N°', 'Produit', 'Montant HT', 'Montant TTC', 'État', 'Créé'].map((h) => (
                  <th
                    key={h}
              ...
              {visible.map((q) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td
                      className="px-4 py-2 text-ink-2"
                      style={{ fontSize: '13px', fontWeight: 400 }}
                    >
                      {q.product_name}
                    </td>
                    <td
                      ...
                      style={{ fontSize: '12.5px', fontWeight: 500, fontVariantNumeric: 'tabular-nums' }}
                    >
                    ...
                    <td className="px-4 py-2">
                      <span
                        className={`inline-flex font-mono px-2 py-0.5 rounded ${st.cls}`}
                        style={{
                          fontSize: '11px',
                          fontWeight: 500,
      ...
      {!loading && visible.length > 0 && (
        <div
          className="mt-3 flex items-center justify-between font-mono text-ink-mute-2"
...
function Kpi({
  ...
  return (
    <div className="bg-paper p-4">
      <div
        className="font-mono uppercase text-ink-muted mb-1.5"
      ...
      <div className="flex items-end justify-between gap-3 mt-2.5">
        <span
          ...
          style={{
            fontSize: '11px',
            fontWeight: 500,
            color: deltaPositive ? 'var(--ok-fg)' : 'var(--err-fg)',
          }}
        >
          {delta}
        </span>
          ...
          {spark.map((n, i) => (
            <span
              ...
              style={{
                height: `${(n / max) * 100}%`,
                minHeight: '2px',
                background: i === spark.length - 1 ? 'var(--brand)' : 'var(--line-2)',
              }}
            />
          ))}
        </div>
